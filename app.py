...
قوانین سخت‌گیرانه (رعایت الزامی):
- فقط از ارقام، اسامی و جزئیاتی استفاده کن که عیناً در «نتایج خام گوگل» زیر آمده‌اند.
- هرگز عددی را حدس نزن، گرد نکن یا از دانش عمومی خودت جایگزین نکن؛ اگر رقمی
  (مثلاً قیمت دقیق یک مدل خاص) در نتایج پیدا نشد، به‌صراحت بنویس «در نتایج جستجو یافت نشد»
  و آن مورد را کنار بگذار، به‌جای ساختن عددی مشابه.
...
- جدول را با سینتکس استاندارد مارک‌داون بنویس. تعداد ردیف‌های جدول را با تعداد مواردی که
  در یادداشت‌های محقق واقعاً و با رقم مشخص آمده‌اند برابر کن؛ اگر یادداشت‌ها فقط برای ۲-۳ مورد
  رقم دقیق دارند، همان ۲-۳ ردیف را بنویس. **هرگز برای رسیدن به تعداد ردیف بیشتر، ردیف یا
  عدد جدید نساز.**
- اگر برای یک سلول از جدول رقم دقیقی در یادداشت‌های محقق وجود ندارد، آن سلول را با
  «نامشخص» پر کن؛ به هیچ عنوان عدد تقریبی یا ساختگی جایگزین نکن.
...
- در متن تحلیل روندها و نتیجه‌گیری هم فقط به ارقام و نکاتی استناد کن که در یادداشت‌های
  محقق آمده؛ به‌جای بیان قطعی برای مواردی که داده کافی ندارند، از عباراتی مثل «بر اساس
  داده‌های موجود» یا «داده کافی برای اظهار نظر قطعی در دسترس نیست» استفاده کن.
